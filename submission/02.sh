# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

transaction=""

# Decode once and store the result
DECODED=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

TXID=$(echo "$DECODED"   | jq -r '.txid')
VOUT_0=$(echo "$DECODED" | jq -r '.vout[0].n')
VOUT_1=$(echo "$DECODED" | jq -r '.vout[1].n')
ADDRESS="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

AMOUNT=0.2

# 2 weeks = 14 days × 144 blocks/day (one block every 10 min)
DAYS=14
BLOCKS_PER_DAY=144
CURRENT_BLOCK=25
BLOCK_TARGET=$(( DAYS * BLOCKS_PER_DAY + CURRENT_BLOCK ))

bitcoin-cli -regtest createrawtransaction \
  '[{"txid":"'"$TXID"'","vout":'"$VOUT_0"'},{"txid":"'"$TXID"'","vout":'"$VOUT_1"'}]' \
  '[{"'"$ADDRESS"'":'"$AMOUNT"'}]' \
  $BLOCK_TARGET